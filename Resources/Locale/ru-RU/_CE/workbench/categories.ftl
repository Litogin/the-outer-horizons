ce-recipe-category-all = Все

ce-recipe-category-materials = Материалы
ce-recipe-category-tile = Пол
ce-recipe-category-tool = Инструменты
ce-recipe-category-weapon = Оружие
ce-recipe-category-armor = Броня
ce-recipe-category-carpet = Ковры
ce-recipe-category-wallpaper = Обои
ce-recipe-category-clothing = Одежда

ce-recipe-category-components = Компоненты
ce-recipe-category-structures = Структуры
ce-recipe-category-electronics = Электроника
ce-recipe-category-power = Энергия
ce-recipe-category-units = Заготовки

ce-recipe-category-unfinished = Не закончил, сорян
